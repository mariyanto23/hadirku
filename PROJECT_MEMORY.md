...
- Admin Registrasi Wajah sudah direview dan diperbaiki:
  ...
  - Flip kamera Registrasi Wajah diberi timeout, jeda singkat setelah stream lama dimatikan, dan fallback ke kamera sebelumnya agar ikon tidak berputar terus atau layar tidak menetap hitam jika browser gagal mengganti kamera.
  - Bug flip kamera yang membuat ikon berputar terus dan preview hitam setelah mirror diperbaiki dengan memastikan perhitungan koordinat mirror overlay hanya berjalan di fungsi gambar overlay, bukan saat overlay dibersihkan.
...
- Admin Presensi Wajah sudah ditambahkan:
  ...
  - Flip kamera Presensi Wajah diberi timeout, jeda singkat setelah stream lama dimatikan, dan fallback ke kamera sebelumnya agar ikon tidak berputar terus atau layar tidak menetap hitam jika browser gagal mengganti kamera.
  - Bug flip kamera yang membuat ikon berputar terus dan preview hitam setelah mirror diperbaiki dengan memastikan perhitungan koordinat mirror overlay hanya berjalan di fungsi gambar overlay, bukan saat overlay dibersihkan.
  ...
  - Ringkasan kelas Presensi Wajah menampilkan `Siswa Kelas`, `Sudah Presensi`, `Belum Presensi`, dan `Siap Dipindai`. Daftar `Belum Presensi` berada di card terpisah di sisi daftar agar guru/admin bisa memantau siswa kelas terpilih yang belum memiliki presensi hari ini tanpa menunggu alpa otomatis. Hitungan ini memakai daftar `student_id` yang sudah punya attendance pada tanggal lokal hari ini, lalu dibandingkan dengan seluruh siswa kelas; jangan bergantung pada alias `withExists` untuk nilai `attended_today`.
